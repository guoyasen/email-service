...
import com.iquantex.email.common.config.KafkaConfig;
...
public class YbKafkaProducer {
     ...
     * 第三种用法:发送标准消息异步无阻塞
     **/
    public void sendToKafkaStandardMessageAsync(EmailEvent emailEvent) {

        producer = new KafkaProducer<String, Object>(kafkaConfig.producerConfigs());

        producer.send(new ProducerRecord<String, Object>(topicName, gson.toJson(emailEvent)), new Callback() {
            @Override
            public void onCompletion(RecordMetadata metadata, Exception exception) {
                if (metadata != null) {
                    log.info("生产消息成功{},发送次数{},checksum：{},offset:{},partition:{},topic:{}", emailEvent.getEmailCode(), emailEvent.getSendNum(), metadata.checksum(), metadata.offset(), metadata.partition(), metadata.topic());
                }
                if (exception != null) {
